 ...
 */
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	randNum = rand() % 4;			/* 1 in 4 (25%) probability of reassignment */
	randQ = (rand() % (NR_SCHED_QUEUES - 3)) + 3;	/* reassigned to random queue ranging 3-NR_SCHED_QUEUES */
	if(q > 2 && randNum == 1)		/* if q is 3+ then reassign 25% of the time to random queue randQ */
		q = randQ;
